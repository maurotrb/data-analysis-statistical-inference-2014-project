...
The project study the relationship between the higher degree obtained by United
...
social survey to verify one of the main topic of these discussions: if more education
is related to a higher personal and family income.
...
We can see that high school degree is prevalent with nearly 50% percent of observations.
...
of 179,000 USD. There are some clear outliers in the upper range of the
distribution.

Moreover, there are 216 observations with missing income values. Filtering them out brings
...
between the mean income in constant dollars of family of United States resident,
when they have a different degree.

Stated more precisely, we formulate a null hypoteses ($H_{0}$) that the mean
family income in constant dollars is equal for all the degree groups, and an
alternative hypotheses ($H_{A}$) that at least one pair of means are different
from each other.
...
We could do a pairwise comparison to compare mean across many groups. But we could
find a difference just by chance, even if there is no difference in the populations.

So we employ a method called **analysis of variance** (ANOVA) that uses a single
hypotheses test to check whether the mean across many groups are equal.

ANOVA finds if the variability in the sample means is so large that it seems
unlikely to be from chance alone. ANOVA uses a test statistic F, which represents
a standardized ratio of variability in the sample means relative to the variability
within the group. The larger the observed variability in the sample means relative
to the within group observations, the larger the F will be and the stronger the
evidence against the null hypotheses.

If we can reject the null hypotheses, we could do a pairwise comparison to find
out which of the groups have different means.
...
Before performing ANOVA we must check three conditions on the data:

* **indipendence**: *Are data indipendent?* -- the data consist in a random sample
  from less than 10% of the population and so they could be considered indipendent
* **approximately normal**: *Have data a normal distribution?* -- the normal
  probability plot for each group are shown in Figure 2; there is some deviation
  from normality especially in the upper quartiles
* **constant variance**: *Is the variability across group about equal?* -- we can
  check the variability with the boxplot in Figure 1; we can see that the variability
  is not similar across groups with the Lt High School with the lower variability
  and Graduate with the highest

The conditions on normality and constant variance are not fully respected.
We compute ANOVA nonetheless, but we report this uncertainty in the results.
...
This mean that the probability of observing a F of 121 or higher, if the null
...
Now we can do a pairwise comparison to find out which groups have different means.
