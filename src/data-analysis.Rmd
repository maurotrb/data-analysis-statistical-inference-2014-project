...
The research uses General Social Survey (GSS) data for the year 2012.
...
for the year 2012 and we give a clearer description to the columns.

```{r research_data, warning=FALSE}
research_data = gss[gss$year == 2012, c(12,27)]
colnames(research_data) <- c("Highest.Degree","Family.Income.Constant.USD")
nrow(research_data)
```

We obtain 1974 cases.

### Summary statistics

```{r summary_graphs, warning=FALSE, fig.width=12, fig.height=4, fig.cap='Summary statistics'}
...
table(research_data$Highest.Degree)
# Frequency table for Highest.Degree
prop.table(table(research_data$Highest.Degree))
```

We can see that High School is prevalent with nearly fifty percent of the cases.

**Family Income** is a continuous numerical variable. We summarize it with mean,
range and quartiles, and with a histogram (see Figure 1).
```{r family_income_summary, warning=FALSE}
summary(research_data$Family.Income.Constant.USD)
```

We can see that the distribution is right skewed and unimodal. Fifty percent of
the cases are in the 16,300-63,200 USD (constant dollar) range, and there is a
maximum value of 179,000 USD. There are some clear outliers.

There are 216 cases with missing income values. Filtering them out brings the
number of cases to 1752. The sample size remains significant for the research.
```{r research_data_comp_cases, warning=FALSE}
...
But the wider interquantile range in the college degree categories and the presence
of outliers in the high school degree ones, means that such a relationship is
...
The research tests the hypotheses that there is a statistical significant
difference between the mean income of persons with different degree.

### Methods

We want to compare means across many groups.
Doing pairwise comparison is treacherous. If we have many groups and do many
comparisons, it is likely that we will eventualy find a difference just by
chance, even if there is no difference in the populations.

A method called **analysis of variance** (ANOVA) that uses a single hypotheses
test to check whether the mean across many groups are equal.

ANOVA answer to the question: is the variability in the sample means so large
that it seems unlikely to be from chance alone?

ANOVA uses a test statistic F, which represents a standardized ratio of variability
in the sample means relative to the variability within the group.
The larger the observed variability in the sample means relative to the
within group observations, the larger the F will be and the stronger the
evidence against the null hypotheses.

We can use the F statistics to evaluate the hypotheses in what is called an F
test. A p-value can be computed from the F statistic using an F distribution.

With ANOVA we formulate a null hypoteses ($H_{0}$) that the mean
*Family income in constant USD* is equal for all the *Highest degree*
($H_{0}: \mu_{LHS} = \mu_{HS} = \mu_{JC} = \mu_{B} = \mu_{G}$) and an alternative
hypotheses ($H_{A}$) that at least one pair of means are different from each other.
...
```{r qqplot_degree, warning=FALSE, fig.width=11, fig.height=2.5, fig.cap='Check data normality in Family.Income.Constant.USD distribution, categorized by Highest.Degree'}
par(mfrow = c(1,5))
...
for (i in 1:5) {
  ...
  qqline(research_data[research_data$Highest.Degree == degrees[i],]$Family.Income.Constant.USD)
}
```
...
pairwise.t.test(research_data$Family.Income.Constant.USD, research_data$Highest.Degree,  p.adj="bonferroni")
```
